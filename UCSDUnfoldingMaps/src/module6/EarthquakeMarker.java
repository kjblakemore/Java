...
public abstract class EarthquakeMarker extends CommonMarker implements Comparable<EarthquakeMarker>
{
	...
	//  Compare earthquakes in decreasing order of magnitude.
	@Override
	public int compareTo(EarthquakeMarker marker) {
		return (int) Math.signum(marker.getMagnitude() - this.getMagnitude());
	}
	...
	// Set color of earthquake marker to indicate depth .
	// Red (0xFFFFFF00) at 700 km to Yellow (0xFFFF0000) at 0 km.
	private void colorDetermine(PGraphics pg) {
		float depth = getDepth();
			
		int colorLevel = (int)(depth * 255.0/700.0) * 0x100;
		int shade = 0xFFFF007F + 0xFF00 - colorLevel;
		int strokeShade = shade - 0x7f;	// slightly darker than fill
			
		pg.fill(shade);
		pg.stroke(strokeShade);
	}
